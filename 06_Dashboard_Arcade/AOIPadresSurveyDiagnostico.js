...
// DIAGNÓSTICO (copiar este archivo solo en el editor de pruebas):
// 1 = FeatureSet mínimo vacío; 2 = geometría con un campo;
// 3 = primeros N campos; 4 = esquema completo con HTML corto; 0 = salida normal.
var diagnosticMode = 1;
var diagnosticFieldLimit = 10;
...
if (diagnosticMode == 2) {
  ...
  var geometryCounter = 1;
  for (var geometryIndex in output.features) {
    var geometryFeature = output.features[geometryIndex];
    Push(geometryTest.features, {
      attributes: { debug_id: geometryCounter },
      geometry: geometryFeature.geometry,
    });
    geometryCounter += 1;
  }
  return FeatureSet(Text(geometryTest));
}
...
if (diagnosticMode == 3) {
  ...
  var effectiveLimit = diagnosticFieldLimit;
  if (effectiveLimit > Count(output.fields)) {
    effectiveLimit = Count(output.fields);
  }
  for (var limitedFieldIndex = 0; limitedFieldIndex < effectiveLimit; limitedFieldIndex++) {
    Push(limitedOutput.fields, output.fields[limitedFieldIndex]);
  }
  for (var limitedFeatureIndex in output.features) {
    var originalFeature = output.features[limitedFeatureIndex];
    var limitedAttributes = {};
    for (var attributeIndex = 0; attributeIndex < effectiveLimit; attributeIndex++) {
      var attributeName = output.fields[attributeIndex].name;
      limitedAttributes[attributeName] = originalFeature.attributes[attributeName];
    }
    Push(limitedOutput.features, {
      attributes: limitedAttributes,
      geometry: originalFeature.geometry,
    });
  }
  return FeatureSet(Text(limitedOutput));
}

if (diagnosticMode == 4) {
  for (var shortHtmlIndex in output.features) {
    output.features[shortHtmlIndex].attributes.html = "HTML de diagnóstico";
  }
}
